...
package AppCore::EmailQueue;
{
	...
	__PACKAGE__->meta({
		...
		db		=> $AppCore::Config::EMAIL_DB      || 'appcore',
		table		=> $AppCore::Config::EMAIL_DBTABLE || 'emailqueue',
	...
	sub send_email
	{
		...
		if($AppCore::Config::EMAIL_ENABLE_DEBUG_FOOTER)
		{
			my $host = `hostname`;
			$host =~ s/[\r\n]//g;
...
.($ENV{HTTP_REFERER} ? "Referer: $ENV{HTTP_REFERER}\n":"")
		...
		$from ||= $AppCore::Config::EMAIL_DEFAULT_FROM || $AppCore::Config::WEBMASTER_EMAIL;
		#print "From:$from\nTo:$to\nSubj:$subject\nText:$text\n";
		#print_stack_trace();
	
		my $email_tmp_dir = $AppCore::Config::EMAIL_TMP_DIR || '/tmp';
	...
	sub transmit
	{
		...
		my $prof = $AppCore::Config::EMAIL_DOMAIN_CONFIG{$domain};
		if(!$prof)
		{
			$self->sentflag(1);
	...
	sub _relay
	{
		...
		my @mx = mx($res, $domain);
		@mx = ($domain) if !@mx;
		if(my $mx_list = $AppCore::Config::EMAIL_MX_OVERRIDES{$domain})
		{
			@mx = @$mx_list;
		}
		#my $success = 0;
