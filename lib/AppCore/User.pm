...
# 				$row->{user_string} = $name;
	...
	sub data#()
	{
		...
		if(!$dat)
		{
			return $self->{_data} = AppCore::DBI::JSONWrapper->_init($self);
		}
		return $dat;
	}
	
# Method: set_data($ref)
# If $ref is a hashref, it creates a new AppCore::DBI::JSONWrapper wrapper and sets that as the data class for this instance.
...
# it sets $ref as the object to be used as the data class.
	sub set_data#($ref)
	{
		...
		if(ref $ref eq 'HASH')
		{
			$self->{_data} = AppCore::DBI::JSONWrapper->new($self,$ref);
		}
...
# 		my $photo_url = 'https://graph.facebook.com/me/picture?type=square&access_token='.$token;
	...
	sub _sync_ad_entry
	{
		...
		AppCore::User::GroupList->retrieve_from_sql('userid='.$user_obj.' and groupid not in (select groupid from user_groups where name="ADMIN" or name like "%Admin%")')->delete_all; # was HelpDesk Admin
